...
namespace FS.Core
{
    ...
    internal sealed class BlockStorage : IBlockStorage
    {
        ...
        public int IndexEntrySize => Constants.IndexEntrySize;

        public int IndexPageSize => Constants.IndexPageSize;

        public int MaxItemsInIndexPage => Constants.MaxItemsInIndexPage;
        ...
        public void ReadBlock(int blockIndex, byte[] buffer)
        {
            ...
            if (!isOpened)
            {
                throw new InvalidOperationException("storage is not opened");
            }
