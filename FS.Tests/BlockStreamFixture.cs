...
namespace FS.Tests
{
    ...
    internal sealed class BlockStreamFixture
    {
        ...
        public void SetUp()
        {
            ...
            A.CallTo(() => provider.Read(0, A<byte[]>._)).Invokes((int position, byte[] buffer) => { Array.Copy(readBuffer, buffer, readBuffer.Length); });
            A.CallTo(() => provider.Read(1, A<byte[]>._)).Invokes((int position, byte[] buffer) => { Array.Copy(readBuffer2, buffer, readBuffer2.Length); });
